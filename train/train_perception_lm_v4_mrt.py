"""
Finetuning only the language model + checking training loss + using MRT from epoch 3 + checking gradient norms per layers
and altogether + higher lora rank
Added gradient clipping and increased the LR
Checking gradient magnitutes in each layer separately
Added ROUGE for risk calculation
"""
...
token_level_timestamp = "2025-11-08_18-52"
...
for split_id in range(1, 2):  # change!
    ...
    model = PeftModel.from_pretrained(
        model, f"thesis/out/{token_level_timestamp}/model_split{split_id}_epoch2"
    ).to("cuda")
    model.train()
