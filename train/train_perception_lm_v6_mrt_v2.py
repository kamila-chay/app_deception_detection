...
NUM_DEVICES = 1 # fixed here # so here the default batch size will still be 8 or whatever it was before, we only wanna change the number of outputs!
GRAD_ACCU_STEPS = 8
MICRO_BATCH = 1
...
for split_id in range(1, 2):  # change!
    ...
    model = PeftModel.from_pretrained(
        model, f"thesis/out/{token_level_timestamp}/model_split{split_id}_epoch8" # diferent for each split
    ).to("cuda")
    model.train()
    ...
    for epoch in range(NUM_EPOCHS):
        print(f"Epoch: {epoch}")
        total_loss = 0
        for i, (input, input_completed, raw_clues, input_completed_opposing) in enumerate(train_dataloader):
            ...
            with torch.inference_mode():
                generated_ids = model.generate(
                    **input,
                    ...
                    do_sample=True,
                    top_k=4,
                    num_return_sequences=(RET_SEQUENCES - 2),
