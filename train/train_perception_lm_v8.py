...
from thesis.utils.utils import set_seed, make_conv_for_reasoning_balanced
...
for split_id in range(1, 4):
    ...
    train_dataset = DolosDataset(
        f"thesis/data/train_fold{split_id}.csv", Path("thesis/data"), "mumin_reasoning_labels_balanced", 
        conv_making_func=make_conv_for_reasoning_balanced
    )
    ...
    val_dataset = DolosDataset(
        f"thesis/data/val_fold{split_id}.csv", Path("thesis/data"), "mumin_reasoning_labels_balanced",
        conv_making_func=make_conv_for_reasoning_balanced
    )
    ...
    if get_rank() == 0:
        ...
        plt.savefig(
            f"thesis/out/{timestamp}/model_split{split_id}_val_losses.png"
        )
    barrier()