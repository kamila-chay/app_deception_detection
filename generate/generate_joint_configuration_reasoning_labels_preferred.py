...
def fill_traits(row: pd.Series) -> List[str]:
    ...
    traits = list()
    traits.append(
        "telling the truth" if str(row["Label"]).strip().lower() == "truth" else "lying"
    )
...
def make_prompt(traits: List[str]) -> str:
    ...
    message = (
        f"Imagine you see a video of a person and you know that the person is {traits[0]}. "
        ...
        "Write a text assessing the person's behavior estimating whether or not "
        ...
        f"Output the text only. Use only the cues given here, nothing else. The output should sound professional "
        "but not too formal. "
...
for i, row in df.iterrows():
    ...
    with open(OUTPUT_DIR / f"{filename}_preferred.txt", "w") as f:
        f.write(response)
