...
for split_id, reference_token_level_epoch in ((1, 8), (2, 1), (3, 3)):
    ...
    model_ref = PeftModel.from_pretrained(
        model_ref, f"thesis/out/{timestamp_token_level}/model_split{split_id}_epoch{reference_token_level_epoch}"
    ).to("cuda")
    model_ref.eval()
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        total_loss = 0
        for i, (input, input_completed, input_completed_opposing) in enumerate(val_dataloader):
